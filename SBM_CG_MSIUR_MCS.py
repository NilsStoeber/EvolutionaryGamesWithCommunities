...
Update-Regel: Geglättete Imitation mit Mutation
...
def stratChange(G, A_adj,f, strategies):
    randType = np.random.uniform()
    if randType < 1/(1+random_drift):
        edge_num = np.random.randint(0,len(edges))
        edge = edges[edge_num]
        i,j = edge[0],edge[1]
        
        comm_i = np.argmax(np.cumsum(N_i) > i)
        comm_j = np.argmax(np.cumsum(N_i) > j)
        if strategies[i] == strategies[j]:
            return f, strategies
        payouts = randomPayout(A_adj, strategies, [i,j])
        
        randChange = np.random.uniform()
        if randChange < F(payouts[0],payouts[1]):
            f[n_strat*comm_i + strategies[i]-1] -= 1
            f[n_strat*comm_i + strategies[j]-1] += 1
            strategies[i] = strategies[j]
        else: 
            f[n_strat*comm_j + strategies[j]-1] -= 1
            f[n_strat*comm_j + strategies[i]-1] += 1
            strategies[j] = strategies[i]
            
        return f, strategies
    else: 
        edge = r.sample(edges, k=1)
        player = edge[0][0]
        comm = np.argmax(np.cumsum(N_i) > player)
        new_strat = np.random.randint(1,n_strat+1)
        f[n_strat*comm + strategies[player]-1] -= 1
...
def simulation(G, N = N, f_init = fStart, StartByComm = True, timeInterval = 0.1, maxduration = 5.0):
    ...
    while duration < maxduration:
        t = np.random.exponential(1/(par_total * (1+random_drift)))
