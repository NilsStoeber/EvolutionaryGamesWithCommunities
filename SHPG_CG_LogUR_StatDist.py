...
Mehr als zwei Communities sind möglich, graphische Darstellung für bis zu drei Communities implementiert
'''
...
states = np.prod(N_i+1)
Pdata = np.zeros((states,n_strat*(n_strat-1)*n+1))
Qdata = np.zeros((states,n_strat*(n_strat-1)*n+1))
        ...
        case 1:
            for c in range(n):
                if state[c] != 0:               # C zu D in Community c  
                    Qdata[k,n_strat*n-c] = q(f,pi,n_strat*c)
                if state[c] != N_i[c]:          # D zu C in Community c  
                    Qdata[k,n-c] = q(f,pi,n_strat*c+1)
            
            Qdata[k,0] = -sum(Qdata[k,:])
        case 2:
            for c in range(n):
                if state[c] != 0:               # C zu D in Community c  
                    Pdata[k,n_strat*n-c] = p(f,pi,n_strat*c)
                if state[c] != N_i[c]:          # D zu C in Community c  
                    Pdata[k,n-c] = p(f,pi,n_strat*c+1)
