...
# Üœbergangsrate, dass jemand aus Community i (=k//2) und Strategie C (k%2==0) bzw D (k%2==1) zur anderen Strategie wechselt
def q(f,pi,k):
    return param_total * p(f,pi,k)

# Üœbergangsraten für den Zustand f
def createQ(f):
...
# Üœbergangsraten für alle Zustände
def createQFull():
...
# Bereinigte Üœbergangswahrscheinlichkeiten für den Zustand f
def createPi(f):
...
# Bereinigte Üœbergangswahrscheinlichkeiten für alle Zustände
def createPiFull():
...
def plotVF(vecscale = 1, plot_NE = plot_nasheq):
    P = createPiFull()
    Q = createQFull()
    
    # Verweilparameter
    Qdiag = -Q[:,-1].reshape((N_i[0]+1,N_i[1]+1))
    # Effektive Veränderungsrate (Verweilparameter * 1-Norm des nächsten Üœbergangs)
