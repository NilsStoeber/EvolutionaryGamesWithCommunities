...
N = 200
N_i = np.repeat(N//n, n)
#N_i = np.array([15,15])       #Option Custom Populationseinstellung
N = np.sum(N_i)
...
# Wahrscheinlichkeit, dass jemand aus Community i (=k//n_strat) und Strategie R/P/S (k%n_strat = 0/1/2)
# zur Strategie l wechselt
def p(f,pi,k,new_strat):
    if (k%n_strat == new_strat) or (f[k] == 0): return 0
    prob = 0
    for j in range(n):
        if j == k//n_strat:
            param = param_intra
...
def q(f,pi,k,new_strat):
    return 1/n_strat*random_drift*N_i.T@par_lambda[k//n_strat]*f[k] + param_total * p(f,pi,k,new_strat)
...
def marginal_dist(dist, community):
    ...
    plt.text(-0.07*N_i[community],1.05*N_i[community],"P")
    plt.text(N_i[community], -0.03*N_i[community], "R")
    plt.text(2.07*N_i[community],1.05*N_i[community],"S")
    plt.show()
